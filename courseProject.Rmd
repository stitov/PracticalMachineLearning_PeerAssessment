...
In cycle fit the model for each participant and store multiClassSummary.
...
Model based on each participant is faster than model based on all data (`r round (timeRFSplit, 1)` mins than `r round (timeRF, 1)` mins) and have the same accuracy `r round (mean (mClSum [, 1]), 4)` comparing with accuracy 0.993 on all dataset.
